...
def do_work(path, db_insert):
	...
	try :
		...
		num = input_original_movie_db(path)
		for keys,values in org_dic.items():
			if (db_insert and num !=0):
				input_db(num, path + "/" + values, keys)
				...
				print ("현재 영화 %s는 DB에 존재 합니다."%(path.split("/")[1]))
				break
...
def get_conn():
	try :
		conn = pymysql.connect(host='localhost', user='root', password='btt1234!',
                	           db='btt', charset='utf8')
...
'''
	insert original_movie info
'''
def input_original_movie_db(name):
	try:
		conn, curs = get_conn()
	
		select_sql = "SELECT * FROM original_movie where original_name=%s"
		curs.execute(select_sql, name)
		result = curs.fetchall()
		if (result == ()):
			date = datetime.datetime.now()
			insert_sql = """insert into original_movie(original_name, original_date) values ( %s, %s)"""
			curs.execute(insert_sql, (str(name), str(date)))
			conn.commit()
			curs.execute(select_sql, name)
			seq_num = curs.fetchall()
			for seq in seq_num :
				print (seq['seq'])		
				return seq['seq'] 
		else : 
			get_sql =  "SELECT seq FROM original_movie where original_name=%s"
			curs.execute(select_sql, name)
			return 0
	except Error as error:
		print ("Error",error)
#	
def input_db(movie_seq, values, keys):
	
	try:
		conn, curs = get_conn()
 #    insert into cert(date, bank_info, account_number, country, IP) values ('2018-08-28', '', 1, '', '' );
		sql = """insert into original_img(original_movie_seq, img_path, img_hash) values ( %s, %s, %s)"""
		value = curs.execute(sql, (str(movie_seq), str(values), str(keys)))
		conn.commit()
	except Error as error :
		print (error)
